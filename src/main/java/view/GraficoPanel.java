...
public class GraficoPanel extends JPanel {
    ...
    private void desenharPontos(Graphics2D g2) {
        ...
        for (Medicao m: dados){
            ...
            //Conversoes para mostrar na tela
            int px = margemEsquerda + (int) (((x - minX) / (maxX - minX)) * areaLarg);
            int py = (altura - margemBase) - (int) (((y - minY) / (maxY - minY)) * areaAlt);

            boolean isOutlier = false;
            if(regressao != null){
                double yPrev = regressao.getB0() + (regressao.getB1() * x);
                double residuoPercentual = ((y -yPrev) / yPrev) * 100.0;
                if(Math.abs(residuoPercentual)>limiteOutlier){
                    isOutlier=true;
                }
            }
            if(isOutlier)
    ...
    private void desenharRetaRegressao(Graphics2D g2) {
        ...
        int areaAlt = altura - margemTopo - margemBase;
        double y1 = regressao.getB0() + (regressao.getB1()*minX);
        int px1 = margemEsquerda;
        int py1 = (altura - margemBase) - (int) (((y1 - minY) / (maxY - minY)) * areaAlt);

        double y2 = regressao.getB0() + (regressao.getB1() * maxX);
        int px2 = largura - margemDireita;
        int py2 = (altura-margemBase) - (int) (((y2-minY)/(maxY-minY)) * areaAlt);
